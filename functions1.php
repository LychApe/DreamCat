...
function themeConfig($form) {
	$logoUrl = new Typecho_Widget_Helper_Form_Element_Text('logoUrl', NULL, NULL, _t('博主头像地址'), _t('在这里填入一个图片 URL 地址, 以在网站标题前加上一个作者头像'));
	$Dec = new Typecho_Widget_Helper_Form_Element_Text('Dec', NULL, NULL, _t('站点首页标语'), _t('填入首页标语'));
	$Decc = new Typecho_Widget_Helper_Form_Element_Text('Decc', NULL, NULL, _t('站点首页博主栏显示文字'), _t('填入一个具有逼格的首页博主栏简介或者说明'));
	$commentszs = new Typecho_Widget_Helper_Form_Element_Text('commentszs', NULL, NULL, _t('文章可评论的字数'), _t('填入一个数字,限制评论的字数'));
	$headbj1 = new Typecho_Widget_Helper_Form_Element_Text('headbj1', NULL, NULL, _t('首页公告栏背景地址'), _t('填入一个图片地址，不填则显示默认背景(地球)'));
	$headbj2 = new Typecho_Widget_Helper_Form_Element_Text('headbj2', NULL, NULL, _t('首页博主栏背景地址'), _t('填入一个图片地址，不填则显示默认背景(地球)'));
	$qq = new Typecho_Widget_Helper_Form_Element_Text('qq', NULL, NULL, _t('您的QQ二维码图片地址'), _t('填写您的QQ二维码图片地址,它将在首页以及其他地方显示，不填则不显示'));
	...
	$form->addInput($headbj1);
	$form->addInput($headbj2);
	$form->addInput($qq);
